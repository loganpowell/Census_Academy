...
- The best Sushi restaurant in the city, but also known for not caring too much for phone ordering customers.
...
## Plot
1) `put!` : We want sushi, so we try phone ordering. First, our call isn't answered (they're busy), since no backpressure (there's no answering machine or online option), our put isn't registered
2) `take!` : Then we go to pick up our food, but since `take!` doesn't park, we have to keep checking to see if the order is ready. Alas, they forgot
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNDcwMjk4MTcsLTQ0OTY3NTkyMCwzMT
