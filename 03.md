...
-  `>!` => `chan` (success) | 
	- 
	- `take!` <= `chan` (no food coming into the takeaway counter) Take away (order at the restaurant ) => Then we go to pick up our food, but since `take!` doesn't park, we have to keep checking to see if the order is ready. Obviously we're not in priority status - so, we figure - we're already here, we might as well just eat here.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTUzMDI2MDI5NywtNDQ5Njc1OTIwLDMxND
