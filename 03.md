...
2) `>!` => `chan` (no takers/buffer): Take away (order at the restaurant ) => Then we go to pick up our food, but since `take!` doesn't park, we have to keep checking to see if the order is ready. Obviously we're not in priority status - so, we figure - we're already here, we might as well just eat here.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYzMDE4NjIyMSwtNDQ5Njc1OTIwLDMxND
