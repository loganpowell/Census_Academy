...
`go-loop`        | mac     | Ex: a repeating sequence of orders (puts) or orders-in (takes)
`chan`           | fnc     | A vehicle that serves to coordinate the food from the cook to the customer withhe plate (buffered = on a [conveyor](https://en.wikipedia.org/wiki/Conveyor_belt_sushi) with the order)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY1OTk3MDgzNywtNDQ5Njc1OTIwLDMxND
