...
-  `>!` => `chan` 
	- success: since we get to stand in line (due to backpreassure via `go` block), we get our order in (FIFO), but! Takeaway orders are only catered to
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk5NDEyNzYwNywtNDQ5Njc1OTIwLDMxND
