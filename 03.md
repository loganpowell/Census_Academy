...
- `take!` <= `chan`  (no buffer) (a takeaway counter, you go up, if there's a cook there, you get your order)
	- success: You're in luck, it's slow and the cook is waiting to put (parked), you grab  it to go!
	- fail: You're in the mood, but say the wildcard counter
-  `<!` <= `chan` (no buffer)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU0NjA1MjY2NiwxNjQ5ODc2MTAwLC00ND
