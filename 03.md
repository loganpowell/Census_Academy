...
## Plot
1) `put!` => `take!` : Phone order => We want sushi, so we try phone ordering. First, our call isn't answered (they're busy), since no backpressure (there's no answering machine or online option), our order isn't registered. We'd have to keep calling, but we're hungry, so we decide to 
2) `>!` => `take!` : Take away (order at the restaurant ) => Then we go to pick up our food, but since `take!` doesn't park, we have to keep checking to see if the order is ready. Obviously we're not in priority status - so, we figure - we're already here, we might as well just eat here.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNTA0OTU3NzMsLTQ0OTY3NTkyMCwzMT
