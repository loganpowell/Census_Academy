...
---              | :---:   | ---
`go`             | mac     | like eating at the restaurant (fifo/fcfs = enables synchronous syntax and back-pressure)
`go-loop`        | mac     | like automatic
...
`chan`           | fnc     | the plate (buffered = on a [conveyor](https://en.wikipedia.org/wiki/Conveyor_belt_sushi) with the order) (handlers)
`put!`           | fnc     | ordering sushi for delivery (no backpressure)
...
`close!`         | fnc     | a "closed" sign on the counter/conveyor that ignores new puts (orders). Food on the conveyor (buffer) is allowed to be taken. If nothing's on the conveyor (empty/un buffer(ed)), customers are told to go home (`nil`)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1NTIyNzgzMTUsLTE5MTExMTA1NTAsLT
EwMjY0OTYwMTYsLTE5NzA1MjM4NCwxNjY3MzAxMDQyLC0xNTMw
MjI0ODkzLC0xODI3NDQ2MDUwLC0yMDE3OTQwOTQ3LDYwMDUxND
Y2LC0xOTI1Njc1NjA3LC0yNTAyNDg3OSw2MjUxNTQ1MywtMTY1
MjkzNTA3MV19
-->