...
2) `>!` => `chan` (success) | `take!` <= `chan` (no food) Take away (order at the restaurant ) => Then we go to pick up our food, but since `take!` doesn't park, we have to keep checking to see if the order is ready. Obviously we're not in priority status - so, we figure - we're already here, we might as well just eat here.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0MzQyNDk2OTMsLTQ0OTY3NTkyMCwzMT
