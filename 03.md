...
-  `>!` => `chan` 
	- success: since we get to stand in line (due to backpreassure via `go` block), we get our order in (FIFO), but! Takeaway orders are only 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMzODcwNjQ4MCwtNDQ5Njc1OTIwLDMxND
