...
2) `take!` : Take away (order at the restaurant ) => Then we go to pick up our food, but since `take!` doesn't park, we have to keep checking to see if the order is ready. Our pSo, we figure, we're already here, we might as well just eat here.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY1MDQzNzE1MSwtNDQ5Njc1OTIwLDMxND
