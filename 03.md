...
-  `>!` => `chan` 
	- success: since we get to stand in line (due to backpreassure via `go` block), we get our order in (FIFO), but! The hostess tells us that takeaway orders are only catered to if no other orders are coming in, so she recommends we just eat at the restaurant.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMxNzE2MzQ1LC00NDk2NzU5MjAsMzE0NT
