...
`put!`           | fnc     | phone order done through a waiter, who could be too busy to take your order (no backpressure). For backpressure, either wrap in a `go` (sit/wait at the sushi bar with the cook) or order at a conveyor (buffer)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMDU1NTk0MjAsLTE5MTExMTA1NTAsLT
