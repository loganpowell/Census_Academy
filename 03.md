...
### Characters:
- customer: puts are orders, takes are food
- chef: puts are food, takes are orders
- Story from the view point of the customer.

Feature          | Mac/Fnc | Analogy
---              | :---:   | ---
`go`             | mac     | like eating at the sushi bar (fifo/fcfs = enables synchronous syntax and back-pressure)
`go-loop`        | mac     | a repeating sequence of orders (puts) or service (takes)
`chan`           | fnc     | the plate (buffered = on a [conveyor](https://en.wikipedia.org/wiki/Conveyor_belt_sushi) with the order)
"handlers"       | fnc     | Used *when you have to wait (park)* (unless `oncaller`. As a customer: 1) If at a coveyor (buffered) for orders (puts), a signal you get when your order was put on the conveyor or 2) if unbuffered, a signal that it was received by the cook.
`put!`           | fnc     | phone order done through a waiter, who could be too busy to take your order (no backpressure = ). For backpressure, either wrap in a `go` (wait/**park** at the sushi bar with the cook) or order at a conveyor (buffer)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI2MzQwNzkyMiwtNjU3MzQzMTksLTE5NT
Y2Nzk4MTEsMzk3MzQwOTYzLC0xOTExMTEwNTUwLC0xMDI2NDk2
MDE2LC0xOTcwNTIzODQsMTY2NzMwMTA0MiwtMTUzMDIyNDg5My
wtMTgyNzQ0NjA1MCwtMjAxNzk0MDk0Nyw2MDA1MTQ2NiwtMTky
NTY3NTYwNywtMjUwMjQ4NzksNjI1MTU0NTMsLTE2NTI5MzUwNz
FdfQ==
-->